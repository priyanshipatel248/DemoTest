...
const Search = () => {
  ...
  const onFetchEventList = async () => {
    const response = await dispatch(getEventListAPI());

    let res = response.payload.data.events;
    
    const updatedEvents = res.map(event => {
      return {
        ...event,
        isFav: false, // or set to true if you want
      };
    });
    let update= updatedEvents.map((event, index) => ({ id: index + 1, ...event }));
    dispatch(setEventData(update));
